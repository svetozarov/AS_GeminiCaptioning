...
class GeminiCaptioningNode:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "GEMINI MODEL": (
                    ("Gemini 2.0 Flash", "Gemini 2.0 Flash-Lite Preview", "Gemini 1.5 Flash", "Gemini 1.5 Flash-8B", "Gemini 1.5 Pro"),
    ...
    def gemini_caption(self, **kwargs):
        log = []
        # Retrieve parameters from kwargs, with defaults for optional strings.
        image = kwargs.get("IMAGE")
        prompt_type = kwargs.get("PROMPT TYPE") or ""
        apy_key_path = kwargs.get("APY KEY PATH") or ""
        gemini_model = kwargs.get("GEMINI MODEL") or "Gemini 1.5 Pro"
        ...
        if isinstance(prompt_length, int) and prompt_length > 0:
            blocks.append("The length of the prompt should be exactly " + str(prompt_length) + " words")
        ...
        # Map the selected Gemini model to the corresponding model name.
        model_mapping = {
        ...
        model_name = model_mapping.get(gemini_model, "gemini-1.5-pro")
        
        try:
            model = genai.GenerativeModel(model_name=model_name)
            log.append(f"Initiating API call to Gemini model '{gemini_model}'...")
